...
BOOL CMatchToolDlg::Match ()
{
	...
	//for (int i = 0; i < iSearchSize; i++)
	{
		...
		double dAngleStep = atan (2.0 / max (iDstW, iDstH)) * R2D;//minmax
		...
		else
		{
			...
			{
				//
				dAngleStep = atan (2.0 / max (pTemplData->vecPyramid[iLayer].cols, pTemplData->vecPyramid[iLayer].rows)) * R2D;//minmax
				vector<double> vecAngles;
